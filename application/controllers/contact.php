    ...
    public function index()
    {
		...
		if ($_POST)
		{
			...
			{// Yes! everything is valid - Send email
				...
				$message = tr('ui_admin.sender').": " . $post->contact_name . "\n";
				$message .= tr('ui_admin.email').": " . $post->contact_email . "\n";
				$message .= tr('ui_admin.phone').": " . $post->contact_phone . "\n\n";
				$message .= tr('ui_admin.message').": \n" . $post->contact_message . "\n\n\n";
				$message .= "~~~~~~~~~~~~~~~~~~~~~~\n";
				$message .= tr('ui_admin.sent_from_website'). url::base();
				// Send Admin Message
