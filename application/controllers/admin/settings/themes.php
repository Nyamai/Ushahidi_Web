...
class Themes_Controller extends Admin_Controller
{
	...
	{
		parent::__construct();
		$this->template->this_page = tr('ui_admin.settings');
	}
	...
	{		
		...
		if ( preg_match( '|'.tr('ui_admin.theme_name').':(.*)$|mi', $theme_data, $theme_name ) )
			$name = $theme = trim(text::html2txt($theme_name[1]));
		else
			$name = $theme = '';

		if ( preg_match( '|'.tr('ui_admin.description').':(.*)$|mi', $theme_data, $description ) )
			$description = trim(text::html2txt($description[1]));
		else
			$description = '';

		if ( preg_match( '|'.tr('ui_admin.demo').':(.*)$|mi', $theme_data, $demo_url ) )
			$demo_url = trim(text::html2txt($demo_url[1]));
		else
			$demo_url = '';

		if ( preg_match( '|'.tr('ui_admin.version').':(.*)|i', $theme_data, $version ) )
			$version = trim(text::html2txt($version[1]));
		else
			$version = '';
			
		if ( preg_match( '|'.tr('ui_admin.author').':(.*)|i', $theme_data, $author ) )
			$author = trim(text::html2txt($author[1]));
		else
			$author = tr('ui_admin.anonymous');
			
		if ( preg_match( '|'.tr('ui_admin.author_email').':(.*)|i', $theme_data, $author_email ) )
