...
class Users_Controller extends Admin_Controller
{
	...
	{	
		...
		$this->template->content->title = tr('ui_admin.manage_users');
		$form = array
	    (
	        'user_id'   => '',
			'action'	=> '',
			...
			if ($post->validate())
	        {
				...
				if ($post->action == 'a') 				// Add/Edit Action
				{
					...
					else 
					{
						...
						$form_saved = TRUE;
						$form_action = strtoupper(tr('ui_admin.added'));
					}
				}
		...
		$this->template->content->roles = array("login"=>tr('ui_admin.login_role'),"admin"=>tr('ui_admin.admin_role'),"superadmin"=>tr('ui_admin.superadmin_role'));
