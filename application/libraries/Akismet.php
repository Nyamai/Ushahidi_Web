...
class Akismet_Core {
	...
	function init($config) {
		...
		if(!$this->_is_valid_api_key($this->api_key)) {
			$this->set_error('AKISMET_INVALID_KEY', tr('libraries.askimet_invalid_apikey'));
		}
	}
	...
	function _connect() {
		if(!($this->con = @fsockopen($this->akismet_server, $this->api_port))) {
			$this->set_error('AKISMET_SERVER_NOT_FOUND', tr('libraries.akismet_cannot_connect'));
		}
	}
	...
	function get_response($request, $path, $type = "post", $response_length = 1160) {
		...
		if($this->con && !$this->is_error('AKISMET_SERVER_NOT_FOUND')) {
			...
			$this->set_error('AKISMET_RESPONSE_FAILED', tr('libraries.akismet_cannot_retrieve'));
		}
